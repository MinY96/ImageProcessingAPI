...
class EvaluationEngine:
    ...
    def run(self, dataset: TestDatasetRecord, request: EvaluationRequest) -> EvaluationRun:
        ...
        if request.fail_on_unlabeled and unlabeled:
            raise EvaluationValidationError(
                f"test dataset contains {len(unlabeled)} unlabeled image(s); label them or set fail_on_unlabeled=false"
            )
        ...
        for item in labeled_images:
            ...
            try:
                ...
                if raw_prediction == request.ok_label:
                    ...
                    raise EvaluationValidationError(
                        f"decision output must be {request.ok_label!r} or {request.ng_label!r}; got {raw_prediction!r}"
                    )
                score = None
        ...
        summary = self._summarize(total_images=len(dataset.images), results=results)
